...
为空洞卷积时，有$k=d*(k-1)+1$，则：$\lfloor$$\frac{n_{in}+2p-d*(k-1)-1}{s}$$\rfloor$+1
## 感受野计算
...
从后往前计算：  
$r_{l-1}=k_{l-1}+s_{l-1}*(r_{l}-1)$  
$pos_{l}=s_{l}*pos_{l+1}+(\frac{k_{l}-1}{2}-p_{l})$  
从前往后计算：  
$j_{l}=j_{l-1}*s_{l}$  
$r_{l}=r_{l-1}+(k_{l}-1)*j_{l-1}$  
$start_{l}=start_{l-1}+(\frac{k_{l}-1}{2}-p_{l})*j_{l-1}$
## 卷积层的参数量、计算量
...
$F_\beta$ score： $F_\beta=(1+\beta^2)\frac{P*R}{(\beta^2*P)+R}$  
